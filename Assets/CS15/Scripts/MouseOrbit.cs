...
public class MouseOrbit : MonoBehaviour {
    public Transform targetFocus; // 初始聚焦点（身体）
    public Transform faceFocus;   // 脸部聚焦点
    ...
    void LateUpdate() {
        ...
        if (targetObj && targetFocus && faceFocus) {
            ...
            Vector3 focusPoint = Vector3.Lerp(targetFocus.position, faceFocus.position, t);
            ...
            if (surfaceColliders != null && surfaceColliders.Length > 0) {
                ...
                Vector3 vdir = Vector3.Normalize(focusPoint - transform.position);
                ...
                foreach (Collider surfaceCollider in surfaceColliders) {
                    if (surfaceCollider.Raycast(new Ray(transform.position - vdir * bounds_MaxSize, vdir), out hitInfo, Mathf.Infinity)) {
                        reqDistance = Mathf.Max(Vector3.Distance(hitInfo.point, focusPoint) + distance, reqDistance);
                        surfaceFound = true;
                    }
                }
                if (surfaceFound) {
            ...
            Vector3 position = rotation * new Vector3(0.0f, 0.0f + height, -cur_distance) + focusPoint;
